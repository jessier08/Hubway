...
// margins and div selection for lines
...
function dataLoaded(err,trips,stations){
    ...
    // tells d3map to look for start and end station
...
////// DATA FILTERING / SORTING //////
    ...
    globalDispatcher.on('changetimeextent',function(extent){
       ...
       // find most freq end stations from top station list
        var cf = crossfilter(trips)
...
////// PLOTTING POPULAR STATION TEXT //////
        if (nestednewData.length==0){
            if (d3.select('.startText')) {d3.select('.startText').remove();}
        };
        ...
        startText
            ...
            .on('mouseover', function(d){  
                ...
                // filter by station # to find end stations for map
                var cf = crossfilter(trips),
                    tripsByThisStationMap = cf.dimension(function(d){
                    return d.startStation});

                tripsByThisStationMap.filter(d.key);

                var tripsByEndStationForMap = cf.dimension(function(d){return d.endStation});

                //now group end stations, creates value on the key
                var tripsGroupByEndStationForMap = tripsByEndStationForMap.group();

                var endStationsForMap = tripsGroupByEndStationForMap.top(10);

                // select dots from popular end stations on map
                for(var z=0;z<endStationsForMap.length;z++){ 
                        d3.select('#stationDot'+endStationsForMap[z].key)
                            .attr('class','greenCircle');
                }

                // filter by station name to find end stations to plot 
                var cf = crossfilter(trips),
                ...
                for(var i=0;i<tripsGroupByEndStationForThis.top(10).length;i++){
                    topEndingforThisString = topEndingforThisString +
                        tripsGroupByEndStationForThis.top(10)[i].key + " " ;
...
////// ALL TRIPS HISTOGRAM WITH BRUSH //////
    ...
    var scaleX = d3.time.scale().domain(timeExtent).range([0,w]),
        scaleY = d3.scale.linear().range([h/1.002,60]),
        axisX = d3.svg.axis()
            .scale(scaleX)
            .orient('bottom')
